    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const data: Array<{ id:number; titulo:string; descricao?:string; status:string }> = await res.json();
                ...
                data.forEach((t: any) => {
                    const item = { id: String(t.id), title: t.titulo ?? "(sem título)", description: t.descricao ?? "" };
                ...
                <nav className={dash.nav}>
                    {[
                        ...
                        <Link
                            key={item.path}
                            href={item.path}
                            className={`${dash.navItem} ${pathname === item.path ? dash.active : ""}`}
                        >
                            {item.name}
                        </Link>
                    ))}
            ...
            <div className={dash.main}>
                <main className={`${dash.content} ${styles.container}`}>
                    <div className={styles.header}>
                        ...
                        <div style={{margin:"8px 0", fontSize:12, opacity:.7}}>
                            debug → A Fazer: {debugCounts.todo} • Em Andamento: {debugCounts.inProgress} • Finalizado: {debugCounts.done}
                        </div>
                    ...
                    {loading ? (
                        <div style={{ textAlign: "center", padding: "2rem" }}>Carregando tarefas…</div>
                    ) : (
